...
#include "ISXEQKissTemplate.h"

// The mandatory pre-setup function.  Our name is "ISXEQKissTemplate", and the class is ISXEQKissTemplate.
...
// our instanced class.
ISXPreSetup("ISXEQKissTemplate", ISXEQKissTemplate);
...
unsigned int ISXEQKissTemplateXML = 0;
...
bool ISXEQKissTemplate::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQKissTemplate Loaded");
	return true;
}

// shutdown sequence
void ISXEQKissTemplate::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQKissTemplateXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQKissTemplateXML);
...
void ISXEQKissTemplate::ConnectServices()
{
...
void ISXEQKissTemplate::RegisterCommands()
{
	// add any commands
...
void ISXEQKissTemplate::RegisterAliases()
{
	// add any aliases
}

void ISXEQKissTemplate::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQKissTemplate::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQKissTemplate::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQKissTemplate::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQKissTemplate::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQKissTemplate::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQKissTemplateService)
	//        pISInterface->ShutdownService(this,hISXEQKissTemplateService);
}

void ISXEQKissTemplate::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQKissTemplateXML
	ISXEQKissTemplateXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQKissTemplateXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQKissTemplateXML, "General"))
	{
